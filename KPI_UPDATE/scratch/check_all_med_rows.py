...
wb_bl = openpyxl.load_workbook(f_bl, data_only=True, read_only=True)
ws_med = wb_bl["Med"]
med_rows = list(ws_med.iter_rows(values_only=True))

print(f"{'Row':<5} | {'Name':<28} | {'Current STK':<18} | {'Ref STK':<18} | {'STK':<8} | {'Current Bank':<22} | {'Ref Bank':<22} | {'Bank':<8}")
print("-" * 140)
...
for r_i, row in enumerate(med_rows[9:], 10):
    if len(row) >= 10:
        ...
        if name and str(name).strip() and not str(name).strip().startswith("CHUỖI") and not str(name).strip().startswith("Tổng"):
            ...
            is_stk_ok = (c_s == ref_stk and c_s != "") if ref else (c_s != "")
            # check bank fuzzy
            is_bank_ok = False
            if ref and ref_bank:
                ...
                is_bank_ok = (c_b not in ["#N/A", "#REF!", ""])
                
            stk_status = "MATCH" if is_stk_ok else "DIFF/ERR"
            bank_status = "MATCH" if is_bank_ok else "DIFF/ERR"
            
            print(f"{r_i:<5} | {str(name):<28} | {str(stk):<18} | {ref_stk:<18} | {stk_status:<8} | {str(c_b):<22} | {ref_bank:<22} | {bank_status:<8}")
