"""
TOOL THEO DÕI, ĐỊNH GIÁ & XUẤT BÁO CÁO GIÁ TUẦN (MEDIGO PRICING ENGINE)
========================================================================
Tính năng chuẩn hóa:
...
STATE_FILE = SCRIPT_DIR / "state_gia_tuan_truoc.csv"
...
def load_lc_mapping(path: Path, app_file_path: Optional[Path] = None) -> pd.DataFrame:
    """Tải bảng mapping data_LC kết hợp thông tin App Medigo từ sheet import_app."""
    ...
    # Nạp thông tin App Medigo từ sheet import_app
    try:
    ...
    # Bổ sung từ file mapping Kiot - MedigoApp.xlsx nếu có
    ...
    # Fallback Tên App & ĐVT App từ Kiot nếu chưa có
...
def run_pricing_pipeline():
    print("=" * 85)
    print("      MEDIGO PRICING ENGINE (MAPPING APP - KIOT - ĐỐI THỦ CHUẨN XÁC)")
    print("=" * 85)
    ...
    if first_run:
        ...
        backup_name = HISTORY_DIR / f"state_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    ...
    output_filename = OUTPUT_DIR / f"Bao_cao_import_price_{timestamp_str}.xlsx"
    ...
    print(f"\n--- [4/5] Xuất file Báo cáo: {output_filename.name} ---")
    ...
    # In mẫu 10 sản phẩm đối thủ đổi giá trên console
    print("\n" + "=" * 125)
