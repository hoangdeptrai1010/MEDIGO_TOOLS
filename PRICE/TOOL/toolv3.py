"""
TOOL V3: CÔNG CỤ THEO DÕI, ĐỊNH GIÁ & XUẤT BÁO CÁO GIÁ TUẦN MEDIGO (TỰ ĐỘNG LOẠI HÀNG TỒN = 0 & KHÔNG NHẬP)
============================================================================================================
Tính năng nâng cấp ở V3:
  1. Tự động nhận diện file PO chi tiết 1 năm (DanhSachChiTietNhapHang_KV*.xlsx / 28-4.xlsx).
  2. Khớp đúng Link + ĐVT đối thủ (Long Châu / Pharmacity / An Khang) bằng Smart Matching.
  3. Bổ sung đầy đủ thông tin App Medigo: 'ID App', 'Tên trên App', 'ĐVT trên App'.
  4. Lấy đầy đủ 'Giá vốn gần nhất', 'Giá vốn cũ', 'Thời gian nhập gần nhất' từ PO 1 năm + lịch sử 22 kỳ.
  5. LỌC BỎ các sản phẩm không còn kinh doanh: (Tồn kho = 0 & Không có PO 1 năm & Có giá bán) ra khỏi danh sách chính, lưu riêng sang sheet 'DS bi loai'.
  6. Ưu tiên lọc đúng các sản phẩm ĐỐI THỦ THAY ĐỔI GIÁ tuần này làm Sheet 1 trình Sếp.
  7. ĐỂ TRỐNG các cột Giá Mới (Giá mới Online, Giá mới offline low, Giá mới offline high...) để người dùng/Sếp điền.
"""
...
STATE_FILE = SCRIPT_DIR / "state_gia_tuan_truoc_v3.csv"
...
def load_historical_cost_prices(master_path: Path, nhap_path: Path) -> pd.DataFrame:
    # 1. Quét 22 chu kỳ lịch sử trong 'update giá nhập'
    ...
    # 2. Quét file nhập hàng chi tiết (DanhSachChiTietNhapHang_KV / 28-4)
    ...
    # Tính đơn giá nhập chuẩn (gồm VAT)
    ...
    multi_po = df_nhap.groupby("Ma_Hang").filter(lambda x: len(x) > 1)
    po_prev = multi_po.groupby("Ma_Hang").nth(-2).set_index("Ma_Hang")["Gia_von_VAT"].to_dict() if len(multi_po) > 0 else {}

    master_costs["gv_moi_po"] = master_costs["Ma_Hang"].map(po_latest)
    master_costs["gv_cu_po"] = master_costs["Ma_Hang"].map(po_prev)
...
# ======================================================================
# 5. PIPELINE CHẠY CHÍNH V3
# ======================================================================
def run_pricing_pipeline():
    print("=" * 90)
    print("      MEDIGO PRICING ENGINE V3 (TỰ ĐỘNG LOẠI BỎ HÀNG TỒN = 0 & KHÔNG CÓ PO)")
    print("=" * 90)
    ...
    nhap_file = find_latest_file(DATA_DIR, ["28-4.xlsx", "*28-4*.xlsx"], exclude_keywords=["mapping", "banggia_med"])
    ...
    print(f"  • File Nhập hàng PO    : {nhap_file.name} (File PO 28-4)")
    ...
    print("\n--- [2/5] Nạp Master Data, Lịch sử Giá Vốn & Smart Matching ---")
    ...
    if first_run:
        print("=> Lần đầu chạy V3: Sử dụng 'BG LC' ghi nhận trước làm baseline đối thủ.")
        snap_prev = pd.DataFrame(columns=["Ma_Hang", "BG_LC_hien_tai"])
    else:
        backup_name = HISTORY_DIR / f"state_backup_v3_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    ...
    # ==================================================================
    # 5. XUẤT BÁO CÁO EXCEL V3 ĐA SHEET
    # ==================================================================
    timestamp_str = datetime.now().strftime("%Y%m%d_%H%M")
    output_filename = OUTPUT_DIR / f"Bao_cao_import_price_V3_{timestamp_str}.xlsx"
    ...
    print(f"\n--- [4/5] Xuất file Báo cáo V3: {output_filename.name} ---")
    with pd.ExcelWriter(output_filename, engine="openpyxl") as writer:
        ...
        # Sheet 9: Toàn bộ danh mục Master đã lọc (Active 3,169 SP)
        df_active[cols_export_template].rename(columns=rename_export).to_excel(
            writer, sheet_name="Toan bo tuan nay (V3)", index=False
        )

    print("\n" + "=" * 125)
    print(" DANH SÁCH 10 MÃ TIÊU BIỂU ĐỐI THỦ THAY ĐỔI GIÁ (V3 - ĐÃ LỌC BỎ HÀNG ẢO)")
    print("=" * 125)
    ...
    print(f"• Danh mục Active sau khi lọc (Sheet 8):               {len(df_active):,} mã")
    print(f"• Danh mục Đã loại bỏ (Sheet 5):                       {len(df_removed):,} mã")
